...
export function OmniBar({ size = 'compact' }: { size?: 'large' | 'compact' }) {
  const { currentAddress, canGoBack, canGoForward, navigate, goBack, goForward } = useBrowser();
  ...
  return (
    <div className={large ? 'relative w-full max-w-xl mx-auto' : 'relative flex-1 flex items-center gap-2'}>
      {!large && (
        <>
      ...
      {results !== null && (
        <div className="absolute top-full mt-2 left-0 right-0 bg-white border border-[var(--line)] rounded-xl shadow-lg z-10">
          ...
          {!searching && results.length === 0 && <p className="p-3 text-sm text-[var(--ink)]/60">No results.</p>}
          {results.map((site) => (
